...
import { useBulkImportVoters } from "../hooks/election.hooks";
...
export function BulkImportVotersForm({ electionId, onCancel, onSuccess }: Props) {
  ...
  const { mutate: importVoters, isPending: isUploading } = useBulkImportVoters(electionId);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    importVoters(
      { file, identifierColumn },
      {
        onSuccess: (data) => {
          toast.success(`Successfully processed ${data.total_processed} voters!`);
          onSuccess?.();
          onCancel();
        }
      }
  ...
  return (
    <div className="bg-white border border-black p-8 shadow-2xl relative overflow-hidden group max-w-lg w-full mx-auto">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        {/* Upload Area */}
        <div
          className={`border-2 border-dashed p-10 text-center transition-all ${file ? 'border-black bg-black/5' : 'border-black/10 hover:border-black/30'
            }`}
        >
          <input
            type="file"
